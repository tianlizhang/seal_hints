...
def batch_run(args, mode, edge_net, node_net, edge_h, node_h, src_ids, tgt_ids, gt, cite_mask=None, neg_bids=None):
    ...
    if args.two_fuse == 1:
        emb_h = node_net.fuse_src(torch.hstack([edge_h[src_ids].detach(), node_h[src_ids]]))
    else:
        emb_h = node_h[src_ids]
    gdim = gt.shape[1]
    pred = node_net.predictor(emb_h)[:, 0:gdim]
    ...
    for ii, tt in enumerate(range(args.train_start + args.node_year, args.train_end)):
        ...
        for perm in tqdm(DataLoader(range(left, right), args.bs, shuffle=False), desc=f'{tt}'):
            ...
            edge_loss, node_loss, pos_out, neg_out, pred = batch_run(args, 'train', edge_net, node_net, \
                edge_h, node_h, src_ids, tgt_ids, gt, cite_mask, neg_edge[perm-left].to(args.device))
...
def main(args):
    ...
    for epoch in range(args.epochs):
        train_rst = train(args, edge_net, node_net, all_g, node_infos, opt_edge, opt_node)
        valid_rst = test(args, edge_net, node_net, all_g, val_test_dcit, node_infos)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--two_fuse', type=int, default=0)
