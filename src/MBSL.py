...
def batch_run(args, mode, edge_net, node_net, edge_h, node_h, src_ids, tgt_ids, gt, cite_mask=None, neg_bids=None):
    ...
    pos_out = edge_net.predictor(src_h, tgt_h) # [b, 1]
    ...
    if mode == 'test':
        ...
        neg_out = edge_net.predictor(src_h, neg_h)
...
@torch.no_grad()
def test(args, edge_net, node_net, valid_g, val_test_dcit, node_infos):
    edge_net.eval()
    node_net.eval()
    
    edge_h = edge_net.gnn(valid_g.x.to(args.device), valid_g.edge_index.to(args.device))
...
def train(args, edge_net, node_net, train_g, node_infos, opt_edge, opt_node):
    edge_net.train()
    node_net.train()
    ...
    for ii, tt in enumerate(range(args.train_start + args.node_year, args.train_end)):
        ...
        source_edge = train_g.edge_index[0, left:right]
        target_edge = train_g.edge_index[1, left:right]
        num_nodes = train_g.edge_index[:, left:right].max().item() + 1
        
        neg_edge = utils.local_neg_sample(torch.stack([source_edge, target_edge]), \
            num_nodes, 1, quick=False)[1, :, 0] # [2, e, 1] -> [e]
        
        for perm in tqdm(DataLoader(range(source_edge.size(0)), args.bs, shuffle=False), desc=f'{tt}'):
            # src_ids, tgt_ids = source_edge[perm].to(args.device), target_edge[perm].to(args.device) # [b]
            src_ids, tgt_ids = source_edge[perm], target_edge[perm]# [b]
            # 1. Get embeds of two graphs
            edge_h = edge_net.gnn(train_g.x, train_g.edge_index)
            ...
            edge_loss, node_loss, pos_out, neg_out, pred = batch_run(args, 'train', edge_net, node_net, \
                edge_h, node_h, src_ids, tgt_ids, gt, cite_mask, neg_edge[perm].to(args.device))
...
def main(args):
    node_infos = data_loader.label_preprocess(args)
    train_g, valid_g, val_test_dcit = data_loader.load_graph(args)
    logger.info(f'train_g: {train_g}, valid_g: {valid_g}')
    ...
    train_g.x = train_g.x.to(args.device)
    train_g.edge_index = train_g.edge_index.to(args.device)
    # train_g = train_g.to(args.device)
    # valid_g = valid_g.to(args.device)
    
    # valid_rst = test(args, edge_net, node_net, valid_g, val_test_dcit, labels_cum_log)
    # logger.info(valid_rst)
    for epoch in range(args.epochs):
        train_rst = train(args, edge_net, node_net, train_g, node_infos, opt_edge, opt_node)
        valid_rst = test(args, edge_net, node_net, valid_g, val_test_dcit, node_infos)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--linkgnn', type=str, default="sage")
