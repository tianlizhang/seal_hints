...
#         return out['train_graph'], out['valid_graph'], out['val_test_dict']
...
#     train_g, valid_g = build_train_valid_graph(args, ts_graph, rest_eind, rest_tsp)
    
#     val_test_dict['val_neg'] = neg_sample(args, val_test_dict['src'], val_test_dict['val_tgt'], valid_g.num_nodes)
#     val_test_dict['test_neg'] = neg_sample(args, val_test_dict['src'], val_test_dict['test_tgt'], valid_g.num_nodes)

#     out = {'train_graph': train_g, 'valid_graph': valid_g, 'val_test_dict': val_test_dict}
#     pkl.dump(out, open(gpath, 'wb'))
#     return train_g, valid_g, val_test_dict
...
def load_graph(args):
    ...
    if os.path.exists(gpath):
        ...
        train_g, valid_g = build_train_valid_graph(args, ts_graph, out['rest_eind'], out['rest_tsp'])
        return train_g, valid_g, out['val_test_dict']
    else:
        val_test_dict, rest_eind, rest_tsp = two_cites_as_valid_test(args, ts_graph)
        train_g, valid_g = build_train_valid_graph(args, ts_graph, rest_eind, rest_tsp)
...
# def build_graph(args): # inputs -> graph
#     gpath = f'../preprocess/data/{args.dataset}/graph.bin'
#     if os.path.exists(gpath):
#         print(f'Loading graph from {gpath}')
#         return dgl.load_graphs(gpath)[0][0]
    
#     inputs = pkl.load(open(os.path.join(args.data_path, 'inputs.pkl'), 'rb'))
...
#     graph.edata['ts'] = torch.tensor(tsp)
    
#     dgl.save_graphs(gpath, [graph])
#     return graph
...
def build_train_valid_graph(args, ts_graph, rest_eind, rest_tsp):
    ...
    torch.all(train_graph.edge_index == valid_graph.edge_index[:, 0:train_graph.edge_index.shape[1]])
    embed()
    return train_graph, valid_graph
